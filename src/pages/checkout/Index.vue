  ...
  <q-page class="v-main">
    <div class="row q-px-none justify-center q-pt-md q-mx-none">
      <div class="col-md-10 col-12 q-px-md q-mx-none text-center">
        <q-toolbar-title class="text-black text-weight-bold text-h6">Checkout</q-toolbar-title>
      </div>
    </div>
        ...
        <div class="row q-col-gutter-x-md q-col-gutter-y-md">
          <div class="col-12">
            <q-card class="card-app">
              <q-card-section>
                <p class="text-info text-normal q-my-xs text-h6 text-weight-bold">{{ getAddress }}</p>
          ...
          <div class="col-12">
            <q-card class="card-app">
              <q-card-section>
                <p class="text-info text-normal q-my-xs text-h6 text-weight-bold">Buy method</p>
                <div class="row">
                  <q-radio
                    v-model="checkMethod"
                    :val="1"
    ...
    <div class="row justify-center q-pt-md q-px-none q-mx-none">
      <div class="col-lg-5 col-md-10 col-12 q-px-sm self-center">
        <q-card class="card-app">
          <q-card-section>
            <p class="text-info text-normal q-my-xs text-h6 text-weight-bold">Price details</p>
            <div class="row">
              <div class="col-6 text-weight-bold text-subtitle2">Total N° of Products</div>
    ...
    <div class="row justify-center q-py-xs q-px-none q-mx-none">
      <div class="col-lg-5 col-md-10 col-12 justify-between align-center q-py-md flex q-px-sm">
        <q-btn
          square
          size="md"
...
import AddresssesDialog from '../../components/dialogs/AddresssesDialog.vue';
  ...
  methods: {
    ...
    async pay() {
      ...
      const options = {
        ...
        "handler": function (response) {
          ...
          self.$refs.paymentFailed.open(razorpay_payment_id, razorpay_order_id, razorpay_signature).then(res => {
            if (res) {
      ...
      rzp1.on('payment.failed', function (response) {
        ...
        self.$refs.paymentSuccessfull.open(
          code,
          description,
          source,
          step,
    ...
    placeOrder() {
      if (this.$store.getters["cart/getQuantityCart"] > 0) {
        this.$refs.confirmPaymentDialog.open().then(res => {
          if (res) {
            //pay now
            this.pay()
          } else {
              ...
              .then(res => {
                ...
                if (res.data.success == true) {
                  this.$refs.paymentSuccessfull.open().then(res => {
                    if (res) {
                      this.$store.dispatch("cart/updateCart", this.$store.getters["auth/getUserId"])
                    }
                  })
                } else {
